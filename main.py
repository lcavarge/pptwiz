...
HEADERS_SS    = {"Content-Type": "application/json",
...
async def gerar_apresentacao(texto: str, slides: int = 5) -> str:
    ...
    async with httpx.AsyncClient(timeout=HTTPX_TIMEOUT) as c:
        ...
        if not task_id:
            return "Erro: task_id vazio (quota ou API key incorreta)"
        ...
        while True:
            try:
                ...
                log("SlideSpeak|timeout, retrying…")
...
async def slack_events(req: Request):
    ...
    log("Slack|payload", {"type": payload.get("type")})

    # handshake
    if payload.get("type") == "url_verification":
        return Response(
            content=f'{{"challenge":"{payload["challenge"]}"}}',
            media_type="application/json")

    event = payload.get("event", {})
    if "bot_id" in event:
        return {"ok": True}        # ignore mensagens do próprio bot

    channel = event.get("channel")
    if event.get("channel_type") == "im":          # DM
        channel = event.get("user")                # usa user_id
